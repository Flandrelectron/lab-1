...
NA means not available, because there are missing values in date set n_2021, so the output will be NA.
...
**Write your answer here**
This represents that there are many attractions that their number of visitors are low and same, and for some attractions their number of visitors are much greater, which increase the mean a lot.
...
```{r Ex_l}
attractions_with_nations <- visitors %>%
  mutate(
    nation = case_when(
       region == "Northern Ireland" ~ "Northern Ireland",
      region == "Scotland" ~ "Scotland",
